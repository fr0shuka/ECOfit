...
class AdminAnalyticsView:
    ...
    def renderizar_view(cls, dados_atividades: list, dados_utilizadores: list):
        ...
        # Tratar dados numéricos de forma defensiva
        ...
        # Cartões KPI Em Linha (st.metric nativo)
        col1, col2, col3, col4 = st.columns(4)

        col1.metric(
            label="Utilizadores",
            value=str(total_utilizadores),
            help="Total de utilizadores registados na plataforma."
        )

        col2.metric(
            label="Atividades",
            value=str(total_registos),
            help="Número total de registos de hábitos efetuados."
        )

        col3.metric(
            label="Pontos",
            value=total_pontos,
            help="Soma de todos os pontos atribuídos na plataforma."
        )

        col4.metric(
            label="Distância",
            value=total_km,
            help="Volume total de quilómetros acumulados por todos os alunos."
        )

        st.markdown("---")

        # Gráfico de Evolução Diária Global
